...
2. **Emitter checks.** Treat each log only as a candidate because another contract can emit the same topic. Deduplicate candidates by `(chainId, tokenContract, tokenId)`. At the pinned block, inspect the emitter's code and ERC-165 responses, decode the ids, and read its balances for the custody address. An event-signature match from a contract that fails the ERC-165 procedure is an anomalous log, not proof of an ERC-1155 balance. A failed required chain read is `DATA_UNAVAILABLE`.
3. **Classification.** The venue profile's contract list classifies candidates; it does not limit discovery. Keep every nonzero balance on an active position contract. A nonzero balance under a named sentinel, or a token acquired through an authorized action before its position profile was active, produces `UNSUPPORTED_POSITION_FORMAT`. An unknown token sent without subject authorization follows the component generation's predeclared unsolicited-token exclusion or return policy. The record still identifies it, and the token cannot enter NAV. Ambiguous provenance produces `UNVERIFIABLE_INVENTORY`. A nonzero supported-contract id whose required identity fields cannot be recovered also produces `UNVERIFIABLE_INVENTORY`.
...
6. **Size floors.** Any de-minimis exclusion (dust positions) MUST be a declared parameter (`minPositionSize`) applied to chain balances in compute, never a venue-API-side filter applied invisibly at capture.
