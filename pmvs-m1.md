...
| M2 | NAV and the zero-NAV decision were computed before physical redemption and never revisited | Zero-NAV on post-redemption state (the false-retirement guard) | closed 2026-08-18: the terminal decision is re-made on post-redemption state; sentinel NAV with positions still held blocks loudly; a positionless estate is required before the zero-NAV wind-down; pinned by a resume-matrix regression harness |
| M3 | Zero-bid write-off from a single observation, bypassing the illiquidity block at any size | Persistence plus materiality-capped write-offs | closed 2026-08-18: write-off allowance capped at 1% of NAV per position and 2% aggregate, the aggregate budget shared with the negligibility skip; a second book read about 5s later must confirm the empty bid side, with a fresh liquidity re-read on flicker; metadata-unlabeled positions never qualify. The spec's fuller declared-interval persistence (`zeroBidObservations`/`zeroBidWindow` as record parameters) remains a target |
